<?php
	...
	{ 
		$sql = "SELECT * FROM tabletickets WHERE TicketNumber = ? AND Status = ? OR TicketNumber = ? AND Status = ?";
		if($stmt = mysqli_prepare($link, $sql))
		{
			$status1 = "ON-GOING";
			$status2 = "PENDING";
			mysqli_stmt_bind_param($stmt,"ssss",$_GET['username'], $status1, $_GET['username'], $status2);
			if(mysqli_stmt_execute($stmt))
			{
				$result = mysqli_stmt_get_result($stmt);
				if(mysqli_num_rows($result) > 0)
				{
					$sql = "UPDATE tabletickets SET AssignedTo = ?, Status = 'ON-GOING', dateAssigned = ? WHERE TicketNumber = ?";
					if($stmt = mysqli_prepare($link, $sql))
					{
						$Date = date('Y-m-d');
						mysqli_stmt_bind_param($stmt, "sss", $_POST['cmbProblem'], $Date, $_GET['username']);
						if(mysqli_stmt_execute($stmt))
						{
							if($stmt = mysqli_prepare($link, $sql))
							{
				...
				{ //loading current value of account
					echo "<b style = 'color:red'>Status is not PENDING or ON-GOING</b>";
	                 	...
	                 	<label>Select Account Assign to:</label>
	                  	<select name = "cmbProblem" id = "cmbProblem" class = "box" required>
                            ...
                            <?php
                            	...
                            	$sql = "SELECT * FROM tableaccounts WHERE username <> ? AND status = 'ACTIVE' AND usertype = 'USER' OR usertype = 'TECHNICAL' ORDER BY username";
							        if($stmt = mysqli_prepare($link, $sql))
							        {
